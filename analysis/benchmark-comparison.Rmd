...
(caller_evo <- combined_results_df %>% 
    ...
    mutate(IDcat = str_c(Submission_Name, challenge_cat),
           IDcat = fct_reorder(IDcat, phred_F1)) %>% 
    filter(Technology == "ILLUMINA") %>% 
    ggplot() + 
    geom_hline(data = top_df, aes(yintercept = phred_F1)) +
    geom_point(aes(x = IDcat, y = phred_F1, fill = Challenge), shape = 21) + 
    ...
    labs(x = "Submission", y = "F1 % (log)", fill = "Challenge Version") +
...
ggplot() + 
    geom_point(aes(x = Submission_Name, y = phred_F1, fill = benchmark_version), 
               shape = 21) + 
    theme_bw() + 

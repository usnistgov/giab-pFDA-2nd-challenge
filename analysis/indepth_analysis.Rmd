...
scatter_plt_df <- wide_strat_results_df %>%
    ...
    mutate(
        rank = 1:n(),
        labeled_strats = case_when(rank < 3 &&
                                       metric_diff > 0 ~ strat_id,
                                   n() - rank < 3 ~ strat_id)
    ) %>%
    mutate(
        metric = str_remove(metric, "METRIC."),
...
(
    scatter_plt <- scatter_plt_df %>%
        ...
        theme_bw() +
        labs(x = "ILL DeepVariant %",
             y = "ONT PEPPER-DeepVariant %",
             fill = "Stratification\nCategory") +
        scale_x_continuous(
        ...
        annotation_phredticks(sides = "lb",
                              scaled = FALSE) +
...
ggsave(here("figures","indepth_analysis.pdf"), scatter_plt, height = 8, width = 6, dpi = "retina")
```
```{r}
